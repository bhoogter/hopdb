...
function hopdb_edit_list($r, $userhoplist=false)
	...
	$s = $s . "<h3>Listing of HoPs</h3>";
	$s = $s . "<ul>\n";
	while ($row = mysql_fetch_assoc($r)) {$c++;$s = $s . " <li><a href='?id=$row[ID]&page=$_REQUEST[page]'>$row[Name] - $row[City], $row[State]</a></li>\n";}
...
function hopdb_entry_form($row, $mode="")
	...
	switch($mode)
		...
		case "useredit":			$s = $s . "<h2>User Edit HoP</h2>\n".hopdb_msg_style(401,false,"<h6>If this is your listing, you will need a password to edit these changes.  Please contact us for your password.</h6>");break;
